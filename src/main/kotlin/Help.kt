...
public const val HELP_PER_PAGE: Int = 6

private const val PAGE_TIMEOUT = 120_000L  // 120 seconds
...
class HelperExtension(bot: ExtensibleBot) : HelpProvider, Extension(bot) {
    ...
    override suspend fun formatCommandHelp(
        ...
        var description = if (longDescription) {
            bot.translationsProvider.translate(command.description)
        } else {
            bot.translationsProvider.translate(command.description).takeWhile { it != '\n' }
        } + "\n"
        ...
        if (command.arguments == null) {
            ...
            arguments += try {
                ...
                argsObj.args.joinToString("\n") {
                    ...
                    desc += "`: "
                    desc += bot.translationsProvider.translate(it.description)
